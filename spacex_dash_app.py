...
for u in spacex_df["Launch Site"].unique():
    options.append({'label': u, 'value': u})
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        title=f'Total sucess launches for all sites')
    else:
...
def get_scatter_chart(entered_site, range):
    filtered_df = spacex_df[spacex_df["Payload Mass (kg)"] >= range[0]]
    filtered_df = filtered_df[filtered_df["Payload Mass (kg)"] <= range[1]]
    if entered_site == 'ALL':
        title = f'Launches for all sites'
    else:
